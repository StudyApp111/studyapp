...
import { Loader2, Lightbulb, CheckCircle2, AlertCircle, Sparkles, RefreshCw, PenLine, HelpCircle, Brain, X, ChevronLeft } from "lucide-react";
...
Return a score (0-100), feedback (2-3 sentences), strengths array (what they did well), and gaps array (what they missed or misunderstood).`;
      ...
      <div className={`flex items-center justify-center p-4 pb-8 min-h-[400px] w-full max-w-full ${isDark ? 'bg-[#0a0a12]' : 'bg-slate-50'}`} style={{ boxSizing: 'border-box', overflowX: 'hidden' }}>
        ...
        >
          <Card className={`backdrop-blur-xl border-2 shadow-2xl overflow-hidden ${isDark ? 'bg-[#12121a]/95 border-purple-500/30' : 'bg-white/95 border-purple-200'}`}>
            <div className="bg-gradient-to-r from-purple-600 to-indigo-600 px-6 py-6 md:py-8 text-center">
              <motion.div
            ...
            <div className="p-5 md:p-6 text-center">
              ...
              <Button
                ...
                className="w-full h-12 md:h-14 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white font-bold text-base md:text-lg rounded-xl shadow-xl"
              >
  ...
  return (
    <div ref={tabContainerRef} className={`relative flex flex-col w-full max-w-full pb-8 ${isDark ? 'bg-[#0a0a12]' : 'bg-slate-50'}`} style={{ boxSizing: 'border-box', overflowX: 'hidden' }}>
      ...
      {/* Clean neutral top bar — matches theme, no competing gradient */}
      <div className={`px-3 py-2.5 md:px-4 flex items-center justify-between border-b ${isDark ? 'bg-[#12121a] border-white/10' : 'bg-white border-slate-200'}`}>
        <button
          onClick={() => setShowSetsList(true)}
          className={`flex items-center gap-1.5 text-xs font-medium px-2.5 py-1.5 rounded-lg transition-colors ${isDark ? 'text-slate-300 hover:bg-white/10' : 'text-slate-600 hover:bg-slate-100'}`}
          title="Back to Feynman sets"
        >
        ...
        <div className="flex items-center gap-2">
          ...
          <span className={`text-xs ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{setCompletedCount} done</span>
        </div>
        <Button
          variant="ghost"
          size="sm"
          onClick={handleRegenerate}
          className={`gap-1.5 h-7 px-2.5 text-xs font-medium ${isDark ? 'text-purple-300 hover:text-purple-200 hover:bg-purple-500/10' : 'text-purple-600 hover:text-purple-700 hover:bg-purple-50'}`}
          title="Generate a new Feynman set"
        >
                ...
                {!showFeedback ? (
                  ...
                  <>
                    ...
                    <Button
                      ...
                      className="w-full h-11 md:h-14 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white font-semibold rounded-xl shadow-lg text-sm md:text-lg"
                    >
                      {isGrading ? (
