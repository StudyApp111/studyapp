  ...
  return (
    <>
      ...
      <div className="h-full flex flex-col bg-gradient-to-br from-purple-50 via-pink-50/30 to-purple-100/40 md:rounded-2xl overflow-hidden">
        ...
        <div className="flex-shrink-0 bg-gradient-to-r from-purple-600 via-purple-700 to-pink-600 px-2.5 py-2 md:px-6 md:py-4 sticky top-0 z-10 shadow-lg">
          <div className="flex items-center justify-between mb-1.5">
            ...
            <Button
              ...
              className="gap-1 bg-white/10 hover:bg-white/20 text-white border-white/20 h-7 px-2 md:h-8 md:px-3"
            >
        ...
        {/* Compact scrollable content for 9:16 */}
        <div className="flex-1 overflow-y-auto px-2 py-2 md:px-6 md:py-4">
          <AnimatePresence mode="wait">
            ...
            >
              <Card className="bg-white/95 backdrop-blur-xl border-2 border-purple-200/50 shadow-2xl overflow-hidden">
                {/* Compact card header */}
                <div className="bg-gradient-to-r from-purple-500 to-pink-500 px-3 py-2.5 md:px-6 md:py-5">
                  <Badge className="mb-1.5 bg-white/20 text-white border-white/30 backdrop-blur-sm text-[10px] md:text-xs">
                    <Sparkles className="w-2.5 h-2.5 md:w-3 md:h-3 mr-1" />
                    Explain This Concept
                  </Badge>
                  <h3 className="text-sm md:text-xl font-bold text-white leading-snug md:leading-relaxed">
                    {currentCard.question}
                  </h3>
                </div>
                  ...
                  {!showFeedback ? (
                    <>
                      <Textarea
                        ...
                        placeholder="Type your explanation... (3-5 sentences)"
                        className="min-h-[140px] md:min-h-[200px] mb-2.5 text-xs md:text-base border-2 border-purple-200 focus:border-purple-400 rounded-xl resize-none"
                        disabled={isGrading}
                      />
                      <Button
                        onClick={gradeAnswer}
                        disabled={!userAnswer.trim() || isGrading}
                        className="w-full h-10 md:h-14 bg-gradient-to-r from-purple-600 via-purple-700 to-pink-600 hover:from-purple-700 hover:via-purple-800 hover:to-pink-700 text-white font-semibold rounded-xl shadow-lg text-xs md:text-lg"
                      >
                        {isGrading ? (
                          <>
                            <Loader2 className="w-3.5 h-3.5 md:w-5 md:h-5 mr-1.5 animate-spin" />
                            Grading...
                          </>
                        ) : (
                          <>
                            <CheckCircle2 className="w-3.5 h-3.5 md:w-5 md:h-5 mr-1.5" />
                            Submit Answer
                          </>
                        )}
                      </Button>
                    ...
                    >
                      ...
                      >
                        <div className={`px-5 py-2 md:px-8 md:py-4 rounded-2xl ${
                          ...
                          <p className="text-white/80 text-[10px] md:text-xs font-medium mb-0.5 text-center">Your Score</p>
                      ...
                      {/* Your Answer */}
                      <div className="bg-purple-50/50 border-2 border-purple-200/50 rounded-lg p-2.5 md:p-4">
                        <h4 className="font-bold text-purple-900 mb-1 md:mb-2 flex items-center gap-1.5 text-[11px] md:text-sm">
                          <NotebookPen className="w-3 h-3 md:w-4 md:h-4" />
                          Your Answer
                        </h4>
                        <p className="text-slate-700 text-[11px] md:text-sm leading-snug md:leading-relaxed">{currentCard.user_answer}</p>
                      </div>

                      {/* Feedback */}
                      <div className="bg-gradient-to-br from-purple-500/10 to-pink-500/10 border-2 border-purple-300/50 rounded-lg p-2.5 md:p-4">
                        <h4 className="font-bold text-purple-900 mb-1 md:mb-2 text-[11px] md:text-sm">💡 AI Feedback</h4>
                        <p className="text-slate-700 text-[11px] md:text-sm leading-snug md:leading-relaxed">{currentCard.feedback}</p>
                      </div>

                      {/* Strengths */}
                      {currentCard.strengths?.length > 0 && (
                        <div className="bg-emerald-50/50 border-2 border-emerald-200/50 rounded-lg p-2.5 md:p-4">
                          <h4 className="font-bold text-emerald-900 mb-1 md:mb-2 flex items-center gap-1.5 text-[11px] md:text-sm">
                          ...
                          <ul className="space-y-1 md:space-y-2">
                            {currentCard.strengths.map((strength, idx) => (
                              <motion.li 
                                ...
                                className="flex items-start gap-1.5 text-slate-700 text-[11px] md:text-sm"
                              >
                      ...
                      {currentCard.gaps?.length > 0 && (
                        <div className="bg-amber-50/50 border-2 border-amber-200/50 rounded-lg p-2.5 md:p-4">
                          <h4 className="font-bold text-amber-900 mb-1 md:mb-2 flex items-center gap-1.5 text-[11px] md:text-sm">
                      ...
                      {/* Model Answer */}
                      <div className="bg-gradient-to-br from-indigo-50 to-purple-50 border-2 border-indigo-200/50 rounded-lg p-2.5 md:p-4">
                        <h4 className="font-bold text-indigo-900 mb-1 md:mb-2 text-[11px] md:text-sm">🎯 Model Answer</h4>
                        <p className="text-slate-700 text-[11px] md:text-sm leading-snug md:leading-relaxed">{currentCard.model_answer}</p>
                      </div>
                    </motion.div>
                  )}
                </div>
        ...
        <div className="flex-shrink-0 bg-white/95 backdrop-blur-xl border-t border-purple-200 px-2 py-2 md:px-6 md:py-4 sticky bottom-0">
          <div className="flex gap-2 md:gap-3 max-w-2xl mx-auto">
            <Button
              variant="outline"
              onClick={handlePrevious}
              disabled={currentCardIndex === 0}
              className="flex-1 h-9 md:h-12 rounded-xl border-2 border-purple-300 hover:bg-purple-50 disabled:opacity-30 font-semibold text-xs md:text-base"
            >
              Previous
            </Button>
            <Button
              ...
              className="flex-1 h-9 md:h-12 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white rounded-xl shadow-lg disabled:opacity-30 font-semibold text-xs md:text-base"
            >
              Next Card
            </Button>
          </div>
