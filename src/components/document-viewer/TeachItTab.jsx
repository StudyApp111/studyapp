...
Return a score (0-100), feedback (2-3 sentences), strengths array (what they did well), and gaps array (what they missed or misunderstood).`;
  ...
  const updateStudyPlanProgress = async (masteredCount) => {
    try {
      ...
      if (justCompleted) {
        setTaskCompleteToast({ show: true, taskType: 'teach_it' });
      }
    } catch (error) {
  ...
  return (
    <>
      ...
      <TaskCompletionToast
        show={taskCompleteToast.show}
        taskType={taskCompleteToast.taskType}
        onComplete={() => setTaskCompleteToast({ show: false, taskType: '' })}
      />
