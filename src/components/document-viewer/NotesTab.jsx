...
import { Loader2, Settings2, RefreshCw, Copy, Check, Zap, Sparkles, BookOpen, GraduationCap, FileText, Download, Notebook, Pencil, ChevronLeft, ChevronRight as ChevronRightIcon, Headphones } from "lucide-react";
...
export default function NotesTab({ lesson }) {
  ...
  // (when the user just uploaded a doc, notes finish generating shortly after
  // they land on the Notes tab — pick them up live).
  useEffect(() => {
    if (!lesson?.id) return;
    const handler = (e) => {
      if (e.detail?.lesson_id === lesson.id) {
        loadNotes();
      }
    };
  ...
  // Poll once after a short delay if notes are missing but the lesson has
  // extracted content — covers the case where the user navigated to Notes
  // while background generation is still in flight.
  const hasUploadedContent = !!(lesson?.file_url || lesson?.file_urls?.length);
  const isWaitingForAutoNotes = initialLoadDone && !note && hasUploadedContent && (lesson?.extracted_content?.length || 0) > 200;
  useEffect(() => {
    if (!isWaitingForAutoNotes) return;
    const interval = setInterval(loadNotes, 4000);
    return () => clearInterval(interval);
  }, [isWaitingForAutoNotes]);
      ...
      {note ? (
            ...
            <div className="flex items-center gap-1 sm:gap-2 flex-wrap justify-between sm:justify-end">
              ...
              <div className="flex items-center gap-1 sm:gap-2">
                ...
                <Button 
                  ...
                  className={`h-8 w-8 sm:h-9 sm:w-9 ${isEditing ? 'bg-purple-500/20 border-purple-500/50 text-purple-600' : (isDark ? 'text-slate-300 border-white/20 hover:bg-white/10' : 'text-slate-600 border-slate-200 hover:bg-slate-50')}`}
                >
                ...
                <Card className={`p-3 sm:p-12 shadow-sm overflow-hidden break-words ${isDark ? 'border-purple-500/30 bg-[#12121a]' : 'border-slate-200 bg-white'}`} style={{ userSelect: 'text', WebkitUserSelect: 'text', overflowWrap: 'anywhere', wordBreak: 'break-word' }}>
                  <EditableNoteContent
                    ...
                    onSave={async (newContent) => {
                      const updated = await base44.entities.LessonNote.update(note.id, { content: newContent });
        ...
        <div className={`overflow-y-auto p-4 ${isDark ? 'bg-[#12121a]' : 'bg-white'}`}>
          <div className="max-w-md mx-auto text-center py-12">
            <div className={`w-20 h-20 rounded-3xl flex items-center justify-center mb-6 mx-auto shadow-xl ${isDark ? 'bg-violet-600/20 shadow-violet-900/50' : 'bg-violet-100 shadow-violet-100'}`}>
              <Loader2 className={`w-10 h-10 animate-spin ${isDark ? 'text-violet-400' : 'text-violet-600'}`} />
            </div>
            <h2 className={`text-2xl font-bold mb-2 tracking-tight ${isDark ? 'text-slate-100' : 'text-slate-900'}`}>Generating your notes…</h2>
            <p className={`text-sm leading-relaxed ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>
              Polly is reading your document and writing beautiful Turbo-style notes with key terms hyperlinked. This usually takes 10-20 seconds.
            </p>
          </div>
        </div>
      ) : (
