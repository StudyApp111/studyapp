...
export default function NotesTab({ lesson }) {
  ...
  // Self-healing auto-generation: if the user lands on Notes and there are
  // none yet but the lesson has uploaded content, kick off generation here.
  // This makes the tab work regardless of whether CreateLesson's background
  // trigger fired successfully (e.g. on older lessons, or if the user navigated
  // away before .then() resolved).
  ...
  const shouldAutoGenerate = initialLoadDone && !note && !isLoading && hasUploadedContent && hasExtractedContent;
  const [autoTriggered, setAutoTriggered] = useState(false);
  useEffect(() => {
    if (shouldAutoGenerate && !autoTriggered) {
      setAutoTriggered(true);
      generateNotes(settings);
    }
  }, [shouldAutoGenerate, autoTriggered]);
