    ...
    <div className={`flex flex-col relative ${isDark ? 'bg-[#0a0a12]' : 'bg-slate-50'}`}>
      ...
      {note ? (
        <div className="flex flex-col">
          ...
          <div className={`flex items-center justify-between gap-2 p-2 sm:p-3 sm:px-4 border-b sticky top-0 z-10 ${isDark ? 'bg-[#12121a] border-white/10' : 'bg-white border-slate-200'}`}>
            ...
            <div className="flex items-center gap-1 sm:gap-2 flex-shrink-0">
              {/* Font Size Controls — now visible on mobile too */}
              <div className={`flex items-center gap-0.5 px-1 py-0.5 sm:px-2 sm:py-1 rounded-lg border ${isDark ? 'border-white/10 bg-white/5' : 'border-slate-200 bg-slate-50'}`}>
                <button 
                  ...
                  className={`px-1.5 sm:px-2 py-0.5 sm:py-1 rounded text-[10px] sm:text-xs font-medium transition-colors ${fontSize === 'sm' ? 'bg-purple-600 text-white' : (isDark ? 'text-slate-400 hover:text-white' : 'text-slate-600 hover:text-slate-900')}`}
                >
                  A
                </button>
                <button 
                  onClick={() => setFontSize('base')} 
                  className={`px-1.5 sm:px-2 py-0.5 sm:py-1 rounded text-xs sm:text-sm font-medium transition-colors ${fontSize === 'base' ? 'bg-purple-600 text-white' : (isDark ? 'text-slate-400 hover:text-white' : 'text-slate-600 hover:text-slate-900')}`}
                >
                  A
                </button>
                <button 
                  onClick={() => setFontSize('lg')} 
                  className={`px-1.5 sm:px-2 py-0.5 sm:py-1 rounded text-sm sm:text-base font-medium transition-colors ${fontSize === 'lg' ? 'bg-purple-600 text-white' : (isDark ? 'text-slate-400 hover:text-white' : 'text-slate-600 hover:text-slate-900')}`}
                >
                  A
                </button>
              </div>
          ...
          <div className="flex gap-0 overflow-hidden">
            ...
            {/* Main Content */}
            <div className={`flex-1 overflow-y-auto p-2 sm:p-6 ${isDark ? 'bg-[#0a0a12]' : 'bg-slate-50'}`} style={{ boxSizing: 'border-box', overflowX: 'hidden', maxHeight: 'calc(100vh - 140px)' }}>
              <div className="w-full max-w-full md:max-w-4xl mx-auto" style={{ boxSizing: 'border-box' }}>
                <Card className={`p-4 sm:p-12 shadow-sm ${isDark ? 'border-purple-500/30 bg-[#12121a]' : 'border-slate-200 bg-white'} ${highlightMode ? 'highlight-active' : ''}`} style={{ userSelect: 'text', WebkitUserSelect: 'text', cursor: highlightMode ? 'text' : 'auto' }}>
