...
Deno.serve(async (req) => {
    try {
        ...
        if (!question_text || !student_answer || !explanation || !course_name) {
            return Response.json({ 
                error: 'Missing required fields',
                required: ['question_text', 'student_answer', 'explanation', 'course_name']
            }, { status: 400 });
        }
        ...
        const gradingPrompt = `You are a teacher for ${course_name} grading a student's work who is in ${student_grade_level || 'their grade level'}. 
...
INPUT (single JSON object):
{
  ...
  "difficulty_index": "${difficulty_index || 'Moderate Exam-Level'}",
  "correct_answer": "${correct_answer.replace(/"/g, '\\"')}",
  "explanation": "${explanation.replace(/"/g, '\\"')}", 
  "assessed_competencies": ${JSON.stringify(assessed_competencies || [])},
  "targeted_misconception": "${(targeted_misconception || 'N/A').replace(/"/g, '\\"')}",
  "student_answer": "${student_answer.replace(/"/g, '\\"')}",
  "student_grade_level": "${student_grade_level || 'N/A'}",
...
• Do not include extra commentary, markdown, or text outside the JSON.`;
        ...
        try {
            ...
            // Add timestamp
            parsedResponse.graded_at = new Date().toISOString();
