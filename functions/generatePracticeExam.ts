...
Deno.serve(async (req) => {
  try {
    ...
    const { lesson_id, focus_topics, target_competency, misconception_addressed, amount, difficulty, question_types, custom_instructions, title: requestTitle } = await req.json();
...
ALL question text and answers MUST be readable as plain text without any LaTeX rendering.`;
    ...
    examTitle = examTitle ? `Practice Test: ${examTitle}` : 'Practice Quiz';
